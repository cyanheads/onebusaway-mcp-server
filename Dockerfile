...
# that are not needed in the final production image. `--omit=peer` excludes the
# framework's optional service and test tiers; direct runtime dependencies remain.
RUN --mount=type=cache,target=/root/.bun/install/cache \
...
RUN mkdir -p /var/log/onebusaway-mcp-server && chown -R bun:bun /var/log/onebusaway-mcp-server
...
ENV LOGS_DIR="/var/log/onebusaway-mcp-server"
